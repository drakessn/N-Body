...
void solve(int ite){
	...
	for(int t=0;t<ite;t++){			
		/**Reinicio de la fuerza total que actua en cada una de las particulas 
		...
		for(int q=0;q<N;q++){
			...
			for (int thread = 0; thread < thread_count; thread++) {
				loc_forces[thread][q][X]= 0;
				loc_forces[thread][q][Y]= 0;
			}
		}
		...
		for(int q=0;q<N;q++){
			...
			for(int k=N-1;q<k;k--){
				...
				loc_forces[rank][q][X]-=force_qkX;
				loc_forces[rank][q][Y]-=force_qkY;
				loc_forces[rank][k][X]+=force_qkX;
				loc_forces[rank][k][Y]+=force_qkY;
			}
		}
